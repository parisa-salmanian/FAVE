...
SUPPORTED_CITIES = {"vaxjo", "malmo", "goteborg", "stockholm", "kalmar", "norrkoping", "uppsala"}


def _normalize_city_key(city: str) -> str:
    import unicodedata
    s = unicodedata.normalize("NFD", (city or "").strip().lower())
...
def create_job(payload: Dict[str, Any]):
    ...
    if _normalize_city_key(city) not in SUPPORTED_CITIES:
        raise HTTPException(
            status_code=400,
            detail=(
                f"unsupported city: {city!r}. Supported: {sorted(SUPPORTED_CITIES)}. "
                f"To add a new city, see CLAUDE.md → 'Adding a new city'."
            ),
        )
