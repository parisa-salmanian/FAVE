...
# Optional category filter set by --cats (None = bake all POI_CATEGORIES).
BAKE_CATS: list[str] | None = None
...
# The shipped matrices carry the 15 nearest POIs per building (schema field
# "n": 15) — the frontend routingMatrix.js consumes up to that many. Keep 15.
TOP_K = 15
...
def bake_city_mode(
    ...
    counts: dict[str, int] = {}
    for cat in (BAKE_CATS or POI_CATEGORIES):
        ...
        if not pois:
            ...
            out_path.write_text(json.dumps(
                {"mode": mode, "cat": cat, "n": TOP_K, "baked_at": _now(),
                 "key": [], "m": [], "v": []},
                separators=(",", ":"),
            ), encoding="utf-8")
            counts[cat] = 0
...
def bake_city(city_key: str, modes: list[str], force: bool, limit: int | None) -> None:
    ...
    # A scoped bake (--cats) must not clobber the counts of categories it did
    # not touch — merge into the existing per-mode maps.
    if idx_path.exists():
        try:
            prev = json.loads(idx_path.read_text())
            for mode, counts in (prev.get("modes") or {}).items():
                merged = dict(counts)
                merged.update(summary["modes"].get(mode, {}))
                summary["modes"][mode] = merged
        except Exception:
            pass
...
def main() -> int:
    ...
    p.add_argument("--cats", nargs="+", default=None,
                   choices=POI_CATEGORIES, help="POI categories (default: all)")
    args = p.parse_args()
